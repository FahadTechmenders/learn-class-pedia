...
import { useBookCategoryMapping } from '../../../../hooks/api/useBookCategoryMapping';

const BookCategoryMapping = () => {
  const {
    loading,
    error,
    categories,
  ...
  useEffect(() => {
    ...
    loadInitialData();
  }, [getAllCategories, getAllBooks, currentPage, pageSize, filterTerm]);
  ...
  const openMappingModal = useCallback(async (category) => {
    ...
    try {
      const bookIds = await getBookCategoryMapping(category.id);
  ...
  const saveMappings = useCallback(async () => {
    ...
    setLoadingMapping(true);
    try {
      await assignBookCategoryMapping(selectedCategory.id, selectedBooks);
      
      const categoryName = selectedCategory.name || selectedCategory.title || 'Category';
      setSuccessMessage(`Successfully updated book assignments for "${categoryName}"`);
      closeMappingModal();
  ...
  }, [selectedCategory, selectedBooks, assignBookCategoryMapping, closeMappingModal]);

  const handlePageChange = useCallback((newPage) => {
    setCurrentPage(newPage);
  }, []);

  // Assigned books should show all assigned items regardless of search
  const assignedBooksList = books.filter(book => selectedBooks.includes(book.id));
  
  // Available books should be filtered by search term
  const availableBooks = books.filter(book => {
    if (selectedBooks.includes(book.id)) return false;
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                  {categories.length} Categories • Manage book assignments for categories
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="p-6 overflow-y-auto max-h-[calc(90vh-180px)]">
              ...
              <div className="mb-6">
                ...
                {loadingMapping ? (
                  <div className="flex flex-col items-center justify-center py-12 bg-gray-50 dark:bg-gray-700 rounded-lg">
                    ...
                    <span className="text-gray-600 dark:text-gray-300">Loading assigned books...</span>
                  </div>
                  ...
                  <div className="text-center py-12 bg-gray-50 dark:bg-gray-700 rounded-lg border border-dashed border-gray-300 dark:border-gray-600">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Search and add books from the available list below</p>
                  </div>
                ) : (
              ...
              <div className="flex items-center justify-between gap-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                <div className="text-sm text-gray-600 dark:text-gray-400">
                  <span className="font-medium">{assignedBooksList.length}</span> book(s) will be assigned to this category
                </div>
                <div className="flex items-center gap-3">
                  <button
                    onClick={closeMappingModal}
...
export default BookCategoryMapping;
