...
import { usePublisherBadgeMapping } from '../../../../hooks/api/usePublisherBadgeMapping';

const PublisherBadgeMapping = () => {
  const {
    loading,
    error,
    publishers,
  ...
  const openMappingModal = useCallback(async (badge) => {
    ...
    try {
      // Load assigned publishers for this badge with single API call
      const publisherIds = await getPublisherBadgeMapping(badge.id);
      // Ensure publisherIds is always an array
  ...
  const assignedPublishersList = filteredPublishers.filter(publisher => selectedPublishers.includes(publisher.id));
  const availablePublishers = filteredPublishers.filter(publisher => !selectedPublishers.includes(publisher.id));
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400 mt-1 flex items-center gap-2">
                  ...
                  <span>Manage badge assignments for publishers</span>
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="p-6 overflow-y-auto max-h-[calc(90vh-180px)]">
              ...
              {availablePublishers.length > 0 && (
                <div className="mb-6">
                  ...
                  <div className="grid grid-cols-1 gap-3 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
                    {availablePublishers.map((publisher, index) => (
                      <div
                        key={publisher.id}
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          <h4 className="font-semibold text-gray-900 dark:text-white truncate group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                            {publisher.fullName || publisher.fullName || 'Unnamed Publisher'}
                          </h4>
                          {publisher.email && (
                            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                              <span className="font-medium">Email:</span> {publisher.email}
                            </p>
                          )}
                        </div>
                        <button
              ...
              <div className="mb-6">
                ...
                {loadingMapping ? (
                  <div className="flex flex-col items-center justify-center py-12 bg-gradient-to-br from-blue-50 to-purple-50 dark:from-gray-700 dark:to-gray-600 rounded-2xl">
                    ...
                    <span className="text-gray-600 dark:text-gray-300 font-medium">Loading assigned publishers...</span>
                  </div>
                  ...
                  <div className="text-center py-12 bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-700 dark:to-gray-600 rounded-2xl border-2 border-dashed border-gray-300 dark:border-gray-600">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Search and add publishers from the available list above</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 gap-3 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
                    {assignedPublishersList.map((publisher, index) => (
                      <div
                        key={publisher.id}
              ...
              <div className="flex items-center justify-between gap-4 pt-6 border-t-2 border-gray-200 dark:border-gray-700">
                <div className="text-sm text-gray-600 dark:text-gray-400">
                  <span className="font-medium">{assignedPublishersList.length}</span> publisher(s) will be assigned to this badge
                </div>
...
export default PublisherBadgeMapping;
